...
filter_cnv_matrix_by_completeness <- function(
    ...
    # amp_mtx is [Amplicons x Cells]. We filter the columns.
...
plot_cnv_genome <- function(
    ...
    if (!is.null(gene_annotation)) {
        ...
        if (lineplot_type == "Genes+amplicons") {
            ...
            # Apply BOLD to chromosome labels
...
#' Prepare and generate CNV Heatmap for a specific region
#'
#' @description Filters the ploidy matrix of a ScIGMA object on specific
#' chromosomes or genes, and calls plot_cnv_heatmap().
#'
#' @param obj A ScIGMA object containing `ploidy.mtx` and `mae`.
#' @param features Selection vector for the X axis (e.g., c("chr1", "chr2") or c("TP53", "KRAS")). If NULL, displays all.
#' @param projection_type Chosen projection type ("Position" or "Genes").
#'
#' @return A plot object (usually from ComplexHeatmap generated by plot_cnv_heatmap).
#' @export
generate_cnv_heatmap_filtered <- function(
    ...
    # --- 1. Ploidy matrix filtering ---
    filtered_ploidy <- obj$ploidy.mtx

    if (!is.null(features) && length(features) > 0) {
        mat_data_tmp <- t(filtered_ploidy)

        # Retrieve information from MultiAssayExperiment
        ...
        # Check if the user selected entire chromosomes
...
#'
#' @description
#' API function to generate the Ploidy profile lineplot in the same 
#' way as in the Shiny module `mod_analysis_CNV`.
#'
#' @param obj A ScIGMA_data object containing the ploidy matrix (obj$ploidy.mtx).
#' @param features Optional vector of chromosomes (e.g., "chr1") or genes (e.g., "TP53") to filter.
#' @param projection_type "Position" or "Genes+amplicons".
#' @param clone The name of the clone to display (default is the first available).
#'
#' @return A plot object (Plotly or standard depending on plot_cnv_genome).
#' @export
